...
use crate::{
    multicore::Worker,
    plonk::{
        better_better_cs::proof::{
            cs::{
                get_from_map_unchecked, ArithmeticTerm, Gate, GateInternal, MainGate, SimpleBitmap,
            },
            lookup_tables::PolyIdentifier,
        },
        domains::Domain,
        fft::cooley_tukey_ntt::{BitReversedOmegas, OmegasInvBitreversed},
    },
    SynthesisError,
};
...
use super::{
    cs::MainGateTerm,
    lookup_tables::{
        AssembledPolynomialStorage, AssembledPolynomialStorageForMonomialForms,
        PolynomialInConstraint, Variable,
    },
    polynomials_new::*,
};

#[derive(Clone, Debug, Hash, Default)]
pub struct Width4MainGateWithDNextNew;

impl Width4MainGateWithDNextNew {
...
impl<E: Engine> GateInternal<E> for Width4MainGateWithDNextNew {
    ...
    fn verify_on_row(
        ...
        let q_const =
            poly_storage.get_poly_at_step(PolyIdentifier::GateSetupPolynomial(name, 5), row);
        let q_d_next =
            poly_storage.get_poly_at_step(PolyIdentifier::GateSetupPolynomial(name, 6), row);
        ...
        for (q, v) in [a_value, b_value, c_value, d_value]
            .iter()
...
impl<E: Engine> Gate<E> for Width4MainGateWithDNextNew {}

impl<E: Engine> MainGate<E> for Width4MainGateWithDNextNew {
    ...
    fn format_term(
        ...
        padding: Variable,
    ) -> Result<(Vec<Variable>, Vec<E::Fr>), SynthesisError> {
        ...
        debug_assert!(
            instance.num_constant_terms <= allowed_constants,
            "must not containt more constants than allowed"
        );
        debug_assert!(
            instance.num_multiplicative_terms <= allowed_multiplications,
            "must not containt more multiplications than allowed"
        );
        debug_assert!(
            instance.terms.len() <= allowed_constants + allowed_multiplications + allowed_linear,
            "gate can not fit that many terms"
        );
            ...
            match term {
                ArithmeticTerm::Product(vars, coeff) => {
                    debug_assert_eq!(
                        vars.len(),
                        2,
                        "multiplicative terms must contain two variables"
                    );
    ...
    fn format_linear_term_with_duplicates(
        ...
        assert!(
            instance.num_multiplicative_terms <= allowed_multiplications,
            "must not containt multiplications"
        );
        debug_assert!(
            instance.terms.len() <= allowed_constants + allowed_multiplications + allowed_linear,
            "gate can not fit that many terms"
    ...
    fn contribute_into_quotient_for_public_inputs(
        ...
        //         continue;
        //     }
        //     block_on(ensure_in_map_or_create_async(&worker,
        //         p,
        //         domain_size,
        //         omegas_bitreversed,
        //         lde_factor,
