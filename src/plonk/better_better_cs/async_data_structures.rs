...
use crate::pairing::*;
use super::async_polynomials::*;
use super::cs::GateInternal;
use super::data_structures::*;

pub struct AsyncAssembledPolynomialStorage<E: Engine> {
...
pub struct AsyncAssembledPolynomialStorageForMonomialForms<E: Engine> {
...
impl<E: Engine> AsyncAssembledPolynomialStorage<E> {
    pub fn get_poly(&self, id: PolyIdentifier) -> Polynomial<E::Fr, Values> {
        match id {
            p @ PolyIdentifier::VariablesPolynomial(..) => {
                self.state_map.get(&p).expect(&format!("poly {:?} must exist", p)).arc_clone()
            },
            p @ PolyIdentifier::WitnessPolynomial(..) => {
                self.witness_map.get(&p).expect(&format!("poly {:?} must exist", p)).arc_clone()
            },
            p @ PolyIdentifier::GateSetupPolynomial(..) => {
                self.setup_map.get(&p).expect(&format!("poly {:?} must exist", p)).arc_clone()
            },
            p @ PolyIdentifier::GateSelector(..) => {
                self.gate_selectors.get(&p).expect(&format!("poly {:?} must exist", p)).arc_clone()
            },
            p @ PolyIdentifier::PermutationPolynomial(..) => {
                self.setup_map.get(&p).expect(&format!("poly {:?} must exist", p)).arc_clone()
            },
            p @ PolyIdentifier::LookupSelector => {
                self.named_polys.get(&p).expect(&format!("poly {:?} must exist", p)).arc_clone()
            },
    ...
    pub fn add_setup_polys(&mut self, ids: &[PolyIdentifier], polys: &[Polynomial<E::Fr, Values>]) {
        assert_eq!(ids.len(), polys.len());
        for (id, poly) in ids.iter().zip(polys.iter()) {
            let proxy = poly.arc_clone();
...
impl<E: Engine> AsyncAssembledPolynomialStorageForMonomialForms<E> {
    ...
    pub fn add_setup_polys(&mut self, ids: &[PolyIdentifier], polys: &[Polynomial<E::Fr, Coefficients>]) {
        assert_eq!(ids.len(), polys.len());
        for (id, poly) in ids.iter().zip(polys.iter()) {
            let proxy = poly.arc_clone();
...
pub struct LookupDataHolder<E: Engine> {
    ...
    pub t_poly_unpadded_values: Option<Polynomial<E::Fr, Values>>,
    pub t_shifted_unpadded_values: Option<Polynomial<E::Fr, Values>>,
    pub s_poly_unpadded_values: Option<Polynomial<E::Fr, Values>>,
    pub s_shifted_unpadded_values: Option<Polynomial<E::Fr, Values>>,
    pub t_poly_monomial: Option<Polynomial<E::Fr, Coefficients>>,
    pub s_poly_monomial: Option<Polynomial<E::Fr, Coefficients>>,
    pub selector_poly_monomial: Option<Polynomial<E::Fr, Coefficients>>,
    pub table_type_poly_monomial: Option<Polynomial<E::Fr, Coefficients>>,
}