...
pub fn read_optional_polynomial_coeffs<F: PrimeField, R: Read, A: Allocator + Clone + Default>(
    mut reader: R
...
pub fn read_optional_polynomial_values_unpadded<F: PrimeField, R: Read, A: Allocator + Clone + Default>(
    mut reader: R
...
pub fn read_polynomials_coeffs_vec<F: PrimeField, R: Read, A: Allocator + Clone + Default>(
    mut reader: R
...
pub fn read_polynomials_values_unpadded_vec<F: PrimeField, R: Read, A: Allocator + Clone + Default>(
    mut reader: R
...
pub fn read_polynomial_coeffs<F: PrimeField, R: Read, A: Allocator + Clone + Default>(
    mut reader: R
) -> std::io::Result<Polynomial<F, Coefficients, A>> {
    let num_values = reader.read_u64::<BigEndian>()?;
    let mut poly_coeffs = Vec::<F, _>::with_capacity_in(num_values as usize, A::default());
...
pub fn read_polynomial_values_unpadded<F: PrimeField, R: Read, A: Allocator + Clone + Default>(
    mut reader: R
) -> std::io::Result<Polynomial<F, Values, A>> {
    let num_values = reader.read_u64::<BigEndian>()?;
    let mut poly_values = Vec::with_capacity_in(num_values as usize, A::default());
    for _ in 0..num_values {
        let el = read_fr(&mut reader)?;
        poly_values.push(el);
    }
