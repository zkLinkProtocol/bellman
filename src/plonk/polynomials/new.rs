...
impl<F: PrimeField, P: PolynomialForm> Polynomial<F, P> {
    pub fn soft_clone(&self) -> Self {
        ...
        Self { coeffs: coeffs, exp: self.exp.clone(), omega: self.omega.clone(), omegainv: self.omegainv.clone(), geninv: self.geninv.clone(), minv: self.minv.clone(), _marker: self._marker.clone() }
    }
...
impl<F: PrimeField> Polynomial<F, Coefficients> {
    ...
    pub fn from_coeffs(coeffs: Vec<F>) -> Result<Polynomial<F, Coefficients>, SynthesisError>{
        Self::from_coeffs_subvec(SubVec::new(coeffs))
    }

    pub fn from_coeffs_subvec(mut coeffs: SubVec<F>) -> Result<Polynomial<F, Coefficients>, SynthesisError>
    {
    ...
    #[inline(always)]
    pub fn break_into_multiples(mut self, size: usize) -> Result<Vec<Polynomial<F, Coefficients>>, SynthesisError> {
        // let mut coeffs = self.coeffs;
        assert_eq!(self.coeffs.buf.len(), self.coeffs.range.len());
        let mut coeffs = unsafe{Arc::get_mut_unchecked(&mut self.coeffs.buf)};
    ...
    pub fn lde_using_multiple_cosets(self, worker: &Worker, factor: usize) -> Result<Polynomial<F, Values>, SynthesisError> {
        ...
        let coeffs = self.into_coeffs();
        let mut results = vec![coeffs; factor];
...
impl<F: PrimeField> Polynomial<F, Values> {
    ...
    pub fn from_values(mut values: Vec<F>) -> Result<Polynomial<F, Values>, SynthesisError>{
        Self::from_values_subvec(SubVec::new(values))
    }
    pub fn from_values_subvec(mut values: SubVec<F>) -> Result<Polynomial<F, Values>, SynthesisError>
    {
    ...
    pub fn from_values_unpadded(values: Vec<F>) -> Result<Polynomial<F, Values>, SynthesisError>{
        Self::from_values_unpadded_subvec(SubVec::new(values))
    }

    pub fn from_values_unpadded_subvec(values: SubVec<F>) -> Result<Polynomial<F, Values>, SynthesisError>
    {
        let coeffs_len = values.len();
    ...
    pub fn split_into_even_and_odd_assuming_natural_ordering(
        ...
        // let even = Polynomial::from_values_subvec(first)?;
        // let odd = Polynomial::from_values(second)?;
...
mod test {
    ...
    fn test_lde_explicit_multicore_validity() {
        ...
        let scalars = make_random_field_elements::<Fr>(&worker, size);
