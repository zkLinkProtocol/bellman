use super::*;
use crate::{pairing::CurveAffine, plonk::fft::cooley_tukey_ntt::bitreverse};
use crate::CurveProjective;
use rand::thread_rng;
type TestConfigs = A100_40GB_2GPU_Test_Configs; // G5_5GB_Testing_Configs;
use std::time::Instant;

pub fn init_manager() -> DeviceMemoryManager<Fr, TestConfigs> {
...
type ComptactBn256 = crate::compact_bn256::Bn256;
fn test_manager_msm() {
    println!("msm");
    let worker = Worker::new();
    let degree = TestConfigs::FULL_SLOT_SIZE;
    let compact_bases = generate_dummy_bases::<ComptactBn256>(&worker, degree);
